...
def cloud_in_pdpfail_main(path_raw_data,path_result):
    main_function('云诊断内销PDP激活失败',path_raw_data, path_result, __read_one_csv_file, __read_csv_directory, __clean_data_all_data)
...
def cloud_in_pdpfail_plot_trend(path_raw_data,path_result):
    ...
    trend_dics_list['省直辖市']=['广东省', '广西壮族自治区', '新疆维吾尔自治区', '河南省', '江苏省', '湖南省', '山东省', '浙江省']

    trend_dics_list['移除正常cause之后大小'] = ['移除正常cause之后大小']

    plot_trend('云诊断内销PDP激活失败',path_raw_data, path_result,trend_dics_list)
