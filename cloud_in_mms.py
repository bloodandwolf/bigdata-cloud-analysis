...
def clean_data(df_csv_datas):
    ...
    df_csv_datas = df_csv_datas.drop(['emmcid','上报时间','异常进程名','进程版本名','进程版本号',
        '异常进程包名','软件系统类型','国家','异常类型','结束电话网络','结束数据网络','isim支持情况',
        'MBN版本信息','VOLTE配置信息','是否volte','呼叫对方号码','保留字段一','保留字段二','异常次数',
        '日志路径','log信息'],axis=1)
...
def cloud_in_mms_main(path_raw_data,path_result):
    main_function('云诊断内销MMS', path_raw_data, path_result, clean_data)

def cloud_in_mms_plot_trend(path_raw_data,path_result):
    sheet_name_list=['SIM卡', '失败原因', '呼入或呼出', '运营商', '电话网络', '发生时间h', '机型', '系统版本', 'PLMN_CS']
    trend_dics_list={}
    trend_dics_list['出现异常的卡']=['卡1','卡2']
    trend_dics_list['失败原因']=['Acquiring network timed out','-----StatusCode====0','Forbidden [ZTEOSE]-----StatusCode====403','Forbidden-----StatusCode====403','MM is abnormal and released.-----StatusCode====400','extension-code-----StatusCode====313','Bad Gateway [ZTEOSE]-----StatusCode====502','Bad Request-----StatusCode====400','Internal Server Error-----StatusCode====500','Internal Server Error [ZTEOSE]-----StatusCode====500','Bad Gateway-----StatusCode====502']
    trend_dics_list['运营商']=['46000','46001','46003','46011']
    trend_dics_list['起呼电话网络'] = ['GSM','LTE','UMTS','UNKNOWN','EDGE','TD_SCDMA','CDMA - 1xRTT','HSDPA','LTE_CA','CDMA - EvDo rev. A','HSPA','CDMA - eHRPD','GPRS','HSPA+','CDMA']
    trend_dics_list['开始数据网络'] = ['GSM','LTE','UMTS','UNKNOWN','EDGE','TD_SCDMA','CDMA - 1xRTT','HSDPA','LTE_CA','CDMA - EvDo rev. A','HSPA','CDMA - eHRPD','GPRS','HSPA+','CDMA']
    ...
    plot_trend('云诊断内销MMS', path_raw_data, path_result, trend_dics_list)
